// src/components/shared/detail/SharedDetailComponents.jsx - Improved reusable detail page components
    ...
    <div className="fixed inset-0 z-50">
      ...
      <div className="flex items-center justify-center h-full p-4">
        <motion.div
          ...
          className="relative z-10 max-h-[90vh] overflow-visible"
...
export const SharedProfile = ({ 
  ...
  return (
    <section className="w-full max-w-[380px] flex-shrink-0">
      <h1 className="text-xl font-semibold leading-none text-[#488BBE] mb-8 sm:mb-10 text-center lg:text-left">
        {title || `Profil ${type === "student" ? "Siswa" : "Karyawan"}`}
      </h1>
      
      {/* Profile Picture - Aligned under title */}
      <div className="flex justify-center lg:justify-start mb-10 sm:mb-12">
      ...
      <div className="flex flex-col w-full">
        <div className="flex gap-8 sm:gap-12">
          {/* Left Column */}
          <div className="flex flex-col space-y-8 w-[100px] flex-shrink-0">
            {fields[0].map((field) => (
              <div key={field.key}>
                <label className="text-xs leading-loose text-zinc-500 block">
                  {field.label}
                </label>
                <p className="mt-3 text-sm sm:text-base leading-none text-neutral-600 break-words">
                  {field.value || "-"}
                </p>
              </div>
            ))}
        ...
        <button 
          onClick={onEdit}
          className="self-end px-1.5 py-1 mt-10 text-xs font-semibold leading-5 text-white bg-[#488BBE] rounded-md min-h-7 w-[82px] hover:bg-[#3399E9] transition-colors"
        >
          Edit
        </button>
      </div>
...
export const SharedDevelopment = ({ 
  ...
  // Check if we have sufficient data or notes
  const hasNotes = profile.progress && profile.progress !== null && profile.progress.trim() !== '';
  const hasSufficientData = mentalHealthHistory && mentalHealthHistory.length > 1;
  
  return (
    <section className="flex flex-col w-full max-w-[700px]">
      {/* Development Section */}
      <h2 className="text-xl font-semibold text-[#488BBE] mb-6">
        Perkembangan {type === "student" ? "Siswa" : "Karyawan"}
      </h2>
      
      <article className="relative flex flex-col px-4 sm:px-8 lg:px-12 py-6 sm:py-8 w-full text-sm leading-5 rounded-xl border border-gray-300 bg-[#FCFCFC] min-h-32 lg:min-h-40 text-neutral-600 mb-8">
        {/* Arrow Icons - Only active if sufficient data */}
        <button 
          className={clsx(
            "absolute left-2 lg:left-4 top-1/2 transform -translate-y-1/2 transition-colors",
            hasSufficientData 
              ? "text-[#3399E9] hover:text-[#488BBE] cursor-pointer" 
              : "text-gray-300 cursor-not-allowed"
          )}
          disabled={!hasSufficientData}
        >
          <span className="material-icons text-lg lg:text-xl">chevron_left</span>
        </button>
        <button 
          className={clsx(
            "absolute right-2 lg:right-4 top-1/2 transform -translate-y-1/2 transition-colors",
            hasSufficientData 
              ? "text-[#3399E9] hover:text-[#488BBE] cursor-pointer" 
              : "text-gray-300 cursor-not-allowed"
          )}
          disabled={!hasSufficientData}
        >
          <span className="material-icons text-lg lg:text-xl">chevron_right</span>
        </button>
        
        {/* Content */}
        <div className="px-6 lg:px-8 flex items-center justify-center h-full">
          {hasNotes ? (
            <p className="text-center">{profile.progress}</p>
          ) : (
            <div className="text-center text-gray-400">
              <span className="material-icons text-4xl mb-2 block">description</span>
              <p>Belum ada catatan perkembangan</p>
            </div>
          )}
        </div>
      </article>
      ...
      <div className="flex overflow-hidden gap-6 pt-6 sm:pt-8 pr-6 sm:pr-8 pb-4 pl-6 text-sm rounded-xl border border-[#535353] bg-zinc-50">
        {/* Y-axis labels - Better positioning */}
        <div className="flex flex-col justify-between text-right w-20 sm:w-24 h-[160px] sm:h-[200px] py-6">
        ...
        <div className="flex flex-col grow text-center text-slate-500">
          <div className="w-full h-[160px] sm:h-[200px] relative">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                ...
                {/* Horizontal dashed grid lines - Properly positioned */}
                <ReferenceLine y={3.2} stroke="#828898" strokeOpacity={0.6} strokeDasharray="6 4" />
                <ReferenceLine y={2.4} stroke="#828898" strokeOpacity={0.6} strokeDasharray="6 4" />
                <ReferenceLine y={1.6} stroke="#828898" strokeOpacity={0.6} strokeDasharray="6 4" />
                <ReferenceLine y={0.8} stroke="#828898" strokeOpacity={0.6} strokeDasharray="6 4" />
          ...
          <div className="flex justify-between self-center mt-3 w-full max-w-[500px] px-4">
            {chartData.map((data, index) => (
              <span key={index} className="text-xs sm:text-sm">{data.month}</span>
            ))}
          </div>
        </div>
      </div>
...
export const Divider = () => (
  <div
    className="hidden lg:block shrink-0 my-auto w-0 h-[500px] lg:h-[600px]"
    style={{
  ...
  return (
    <main className="overflow-hidden bg-white min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <aside className="hidden lg:block w-[17%] flex-shrink-0 transition-all duration-300" />
        
        {/* Main Content - Responsive */}
        <div className="w-full lg:w-[83%] flex-1 px-4 sm:px-6 lg:px-8 xl:px-12">
          <div className="flex flex-col w-full max-w-7xl">
            {/* Content Layout */}
            <div className="flex flex-col lg:flex-row lg:items-start gap-8 sm:gap-10 lg:gap-[60px] mt-8 sm:mt-12 lg:mt-20 pb-8">
              {children}
            </div>
          </div>
        </div>
