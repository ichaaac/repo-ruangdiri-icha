// src/components/shared/layout/Sidebar.jsx - FINAL REVISION V3
...
/**
 * Responsive Sidebar Component with fixed profile hover and dashboard dropdown animation
 */
const Sidebar = ({
  expanded,
  setExpanded,
  ...
  const handleSidebarMouseLeave = () => {
    if (!expanded && !isMobile) {
      ...
      collapseTimeoutRef.current = setTimeout(() => {
        ...
        onHoverChange?.(false)
      }, 300) // Back to 300ms since we're extending the hover area
    }
  }
  ...
  const toggleSidebar = () => {
    ...
    // Close all dropdowns when manually toggling
    setShowProfileDropdown(false)
    if (!location.pathname.includes("/dashboard")) {
      setActiveDropdown(null)
    }
  ...
  return (
    ...
    >
      {/* Invisible hover extension area for better UX */}
      {!expanded && !isMobile && (
        <div 
          className="absolute top-0 -right-3 w-3 h-full z-50"
          onMouseEnter={handleSidebarMouseEnter}
          onMouseLeave={handleSidebarMouseLeave}
        />
      )}
      ...
      {/* Profile Section */}
      <div 
        className={`${isMobile ? "px-3 mt-10" : "px-2.5 mt-16"} relative`}
        onMouseEnter={handleSidebarMouseEnter}
        onMouseLeave={handleSidebarMouseLeave}
      >
        <div
      ...
      {/* Divider */}
      <div 
        className={`${isMobile ? "mt-6 px-2" : "mt-8 px-3"}`}
        onMouseEnter={handleSidebarMouseEnter}
        onMouseLeave={handleSidebarMouseLeave}
      >
      ...
      {/* Navigation Menu */}
      <div 
        className={`flex flex-col ${isMobile ? "mt-4 gap-y-1" : "mt-6 gap-y-1"} flex-1 overflow-y-auto`}
        onMouseEnter={handleSidebarMouseEnter}
        onMouseLeave={handleSidebarMouseLeave}
      >
