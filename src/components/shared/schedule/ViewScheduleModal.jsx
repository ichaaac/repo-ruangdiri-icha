...
const eventTypes = [
  ...
  const getClients = () => {
    if (scheduleData.type === "counseling") {
      ...
      if (scheduleData.usersSchedules) {
        const clients = scheduleData.usersSchedules.filter((us) => us.user.role !== "psychologist").map((us) => us.user)
  ...
  const handleDelete = () => {
    if (onDelete && !loading && !isLoadingDetail) {
      // The check for isScheduleInPast() is now handled within the toast action if onDelete is called.
      toast("Apakah Anda yakin ingin menghapus jadwal ini?", {
        ...
        action: {
          label: "Ya, Hapus",
          onClick: () => {
            // FIXED: Re-add the past schedule check here to prevent actual deletion
            if (isScheduleInPast()) {
    ...
    >
      <div className="bg-white rounded-lg max-w-[600px] w-full mx-4 max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="flex justify-between items-center p-6">
          <div className="flex-1">{/* Removed commented-out status indicators */}</div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex gap-4 items-center">
            ...
            <div className="flex-1">
              <div className="text-lg font-semibold text-gray-900">{scheduleData.agenda || "No Title"}</div>
            </div>
          ...
          <div className="flex gap-4 items-start">
            ...
            <div className="flex-1">
              ...
              <div className="mt-6 flex gap-3 justify-end">
                {/* Delete button always visible and clickable, but toast prevents action for past schedules */}
                <button
                  onClick={handleDelete}
                  disabled={loading} // Only disable if loading, not based on past schedule
                ...
                {/* FIXED: Edit button with proper disable state and dark color */}
                <button
                  onClick={handleEdit}
                  disabled={loading || isEditDisabled()}
                  className={`px-6 py-2 rounded-lg transition-colors font-medium ${
                    isEditDisabled()
                      ? "bg-[#55555580] text-gray-300 cursor-not-allowed" // Darker disabled style with specified color
                ...
                >
                  {loading ? "Loading..." : "Edit"} {/* Always show "Edit" text */}
                </button>
              </div>
            </div>
          </div>
