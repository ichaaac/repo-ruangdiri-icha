 ...
 * Hanya memiliki akses ke Chat dan Profile
 */
const PsychologistSidebar = ({
  ...
  // Set profile dropdown state based on current page
  useEffect(() => {
  ...
  // Handle closing dropdowns when sidebar collapses
  useEffect(() => {
    if (!expanded && !hovered) {
      const isProfilePage = location.pathname.includes('/profile') || location.pathname.includes('/settings');
      if (!isProfilePage) {
            ...
            >
              <div className={`${isMobile ? "w-8 h-8" : "w-10 h-10"} rounded-full overflow-hidden flex-shrink-0 transition-all`}>
                {(userData?.profilePictureUrl || userData?.profilePicture) && !fallbackProfileImage ? (
                  <img src={userData.profilePictureUrl || userData.profilePicture} alt="Psychologist" className="w-full h-full object-cover" onError={handleImageError} />
                ) : (
              ...
              <motion.div className="overflow-hidden transition-all"
                ...
                transition={{ duration: 0.3, ease: "easeInOut" }}>
                <div className={`${isMobile ? "text-xs" : "text-sm"} font-medium text-[#488BBE]`}>Psikolog</div>
          ...
          <div className={`flex flex-col gap-y-1`}>
            {menuItems.map((item, index) => (
              <div key={index}>
                ...
                >
                  <span className={`material-icons ${isMobile ? "w-[16px]" : "w-[20px]"} flex-shrink-0`} style={{ fontSize: isMobile ? "16px" : "20px" }}>
                    {item.icon}
                  </span>
                  <motion.span 
                    animate={{ 
