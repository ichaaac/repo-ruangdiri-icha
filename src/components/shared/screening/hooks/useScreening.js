...
export const useScreening = () => {
  ...
  const isAllQuestionsAnswered = useCallback(() => {
    return answers.every(answer => answer !== null && answer !== undefined && Number.isInteger(answer) && answer >= 0 && answer <= 3);
  }, [answers]);

  // Get answered questions count
  const getAnsweredCount = useCallback(() => {
    return answers.filter(answer => answer !== null && answer !== undefined && Number.isInteger(answer) && answer >= 0 && answer <= 3).length;
  }, [answers]);
  ...
  const submitScreening = useCallback(async (notes = "") => {
    try {
