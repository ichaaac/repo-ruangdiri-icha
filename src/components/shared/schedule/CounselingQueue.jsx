...
// Local truncate function for CounselingQueue
      ...
      {show && (
        <motion.div
          ...
          style={{
            ...
            display: "-webkit-box",
            webkitLineClamp: 3, // Max 3 lines before ellipsis
...
const CounselingQueue = ({
  ...
  queueData = [], // Real data from the backend
  loading = false,
}) => {
  ...
  // State to control tooltips for location and description
  const [hoveredLocation, setHoveredLocation] = useState(null);
  const [hoveredKeterangan, setHoveredKeterangan] = useState(null);

  // Helper to get color based on screening status
  const getCategoryColor = (status) => {
    switch (status) {
      case "at_risk":
        return "#EE4266"; // Red for "Berisiko"
      case "monitored":
        return "#FFA600"; // Orange for "Pengawasan"
      case "stable":
        return "#9BCA61"; // Soft green for "Stabil"
      default:
        return "#535353"; // Default text color
    }
  };

  useEffect(() => {
    // Set the real data from the backend
                  ...
                  return (
                    <div key={item.id || index}>
                      <div className="px-9 py-3 hover:bg-gray-50 transition-colors">
                        <div className="grid grid-cols-6 gap-4 lg:gap-8 items-center">
                          <div className="text-sm font-normal text-[#535353] whitespace-nowrap overflow-hidden text-ellipsis">
